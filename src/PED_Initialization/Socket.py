...
class SocketHandler:
    ...
    def config_menu(self):
        ...
        '''
        print('* Enter IP address (press enter for local host): ')
        try: 
            self.ip_address = raw_input
        except NameError: 
            pass        
        if not self.ip_address:
            self.ip_address = get_local_host()
        else:
            try:
                socket.inet_aton(self.ip_address)
            except socket.error:
                self.ip_address = ""
                print colored('[ERROR] Invalid IP address', 'yellow')
        if self.ip_address:
            print colored('[INFO] Ip address: {}' .format(self.ip_address), 'green')
        '''
        try:
